...
void printFlashPartition(){
  ...
  while(partitionIt!=NULL){
    ...
    ESP_LOGW(__FUNCTION__, "Partition %s", partitionInfo->label);
    esp_err_t errCode=nvs_flash_init_partition(partitionInfo->label);
    if(errCode!=ESP_OK){
      ESP_LOGW(__FUNCTION__, "Partition %s could not be inited. Error 0x%x, skipping", partitionInfo->label, errCode);
    ...
    if(esp_partition_get_sha256(partitionInfo, partitionSha256)==ESP_OK){
      ESP_LOGD(__FUNCTION__, "SHA256 "SHA256_PRINTF_STR_HEX, SHA256_PRINTF_STR(partitionSha256));
    }

    nvs_stats_t nvsStats;
    ESP_ERROR_CHECK(nvs_get_stats(partitionInfo->label, &nvsStats));
    ESP_LOGI(__FUNCTION__, "%d namespace, %d/%d entries, %d free", nvsStats.namespace_count, nvsStats.used_entries, nvsStats.total_entries, nvsStats.free_entries);
    ...
    while(entryIt!=NULL){
      ...
      ESP_LOGI(__FUNCTION__, "%s:%s, %s", entryInfo.namespace_name, entryInfo.key, nvsTypeToString(entryInfo.type));
...
//  memcpy(coreDumpReport->applicationelfsha256.data, appDesc->app_elf_sha256, coreDumpReport->applicationelfsha256.len);
  ...
  if(esp_core_dump_image_get(&coreAddr, &coreSize)==ESP_OK){
    ESP_LOGD(__FUNCTION__, "coreAddr=0x%x coreSize=%d", coreAddr, coreSize);
  }
  ...
  sprintf(topic, topicFormat, appDesc->project_name, "coredumpsize");
  ...
  if(partitionIt==NULL){
    ESP_LOGE(__FUNCTION__, "Did not find the coredump partition");
    return;
  }
  const esp_partition_t* partitionInfo=esp_partition_get(partitionIt);
    ...
    if(startOfCoreDump==NULL){
      ESP_LOGE(__FUNCTION__, "Did not find the start of coredump");
      return;
    }

//    ESP_LOG_BUFFER_HEX_LEVEL(__FUNCTION__, startOfCoreDump, 1024, ESP_LOG_INFO);
    sprintf(topic, topicFormat, appDesc->project_name, "coredump");
    ...
    ESP_LOGD(__FUNCTION__, "Number of Task=%d", coredumpHeader->tasks_num);
    ESP_LOGD(__FUNCTION__, "Data len=%d", coredumpHeader->data_len);
    ESP_LOGD(__FUNCTION__, "Task Control Block Size=%d", coredumpHeader->tcb_sz);
...
//    coreDumpReport->taskcoredumps=malloc(coreDumpReport->n_taskcoredumps*sizeof(FMDTaskCoreDumpReport*));
    ...
    for(int i=0; i<coredumpHeader->tasks_num; i++){
      ...
      ESP_LOGD(__FUNCTION__, "Task %p Stack TCBaddress=%p start-end=0x%x-0x%x stackSize=%d", taskHeader, taskHeader->tcb_addr, taskHeader->stack_start, taskHeader->stack_end, stackSize);
      ...
      //    ESP_LOGD(__FUNCTION__, "programCounter = %p", taskCoreDump->programCounter);
      ...
      sprintf(topic, topicFormat, appDesc->project_name, "programcounter");
